    ...
    methods: {
        ...
        dragging: function(e) {
            if (this.isDragging) {
                ...
                this.innerProgress =
                    this.draggingOriginProgress + ((e.clientX - this.draggingOriginPoint) / parseInt(width)) * 100
        ...
        jumpProgress: function(e) {
            this.innerProgress = (e.offsetX / parseInt(getComputedStyle(this.$refs.track).width)) * 100
            this.$emit('progressChanged', this.innerProgress)
        }
    }
}
...
.handler {
    ...
    width: 8px;
    top: 3px;
    border: 2px solid rgb(37, 37, 175);
    border-radius: 50%;
    background-color: white;
    z-index: 3000;
    transform: translate(-50%, -6.5px); /*  居中 */
}
